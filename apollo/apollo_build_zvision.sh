...
function generate_build_targets() {
  ...
  drivers)
    ...
    ;;
  zvision_drivers)
    BUILD_TARGETS=`bazel query //cyber/... union //modules/tools/visualizer/... union //modules/drivers/zvision/...`
    #echo -e "${YELLOW}BUild zvision_drivers, targets is ${BUILD_TARGETS}.${NO_COLOR}"
    ;;
  control)
...
function build() {
  ...
  if [ "$BUILD_FILTER" == 'cyber' ] || [ "$BUILD_FILTER" == 'zvision_drivers' ] || [ "$BUILD_FILTER" == 'drivers' ]; then
    info "Skipping revision recording"
    #echo -e "${RED}Build zvision drivers.${NO_COLOR}"
  else
    bazel build $JOB_ARG $DEFINES -c $@ $BUILD_TARGETS
    if [ ${PIPESTATUS[0]} -ne 0 ]; then
      fail 'Build failed!'
...
function print_usage() {
  ...
  ${BLUE}build_zvision [dbg|opt]${NONE}: build zvision drivers only
